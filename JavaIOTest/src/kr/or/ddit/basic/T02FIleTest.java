...
public class T02FIleTest {
	public static void main(String[] args) throws IOException {
		...
		if(f1.exists()) {
			...
			System.out.println(f1.getAbsolutePath() + "은 존재하지 않는 파일입니다.");			
		...
		if (f2.exists()) {
			...
			System.out.println(f2.getAbsolutePath() + "은 존재하지 않는 파일입니다.");
		}
		System.out.println("++++++++++++++++++++++++++++");
	...
	/*
	 지정된 디렉토리(폴더)에 포함된 파일과 디렉토리 목록을 보여주는 기능 수행 메소드
	dir 파일과 디렉토리 목록을 보여주고자 하는 파일 객체 
	 */
	private static void displayFileList(File dir) {
		...
		//하위 디렉토리 정보를 저장할 List 생성(인덱스 정보 저장용)
		...
		for(Integer i : subDirList) {
			displayFileList(files[i]); //재귀호출 메소드에서 메소드 자신을 호출함
		}
	}
}
