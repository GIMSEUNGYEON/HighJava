...
/*
 락(Lock) 기능을 제공하는 클래스 
 - ReentrantLock : Read 및 Write 구분 없이 사용하기 위한 락 클래스로 동기화처리를 위해 사용함.
	 			   synchronized를 이용한 동기화처리보다 부가적인 기능이 제공됨.
	 			   ex) Fairness 설정 등 => 가장 오래 기다린 스레드가 가장 먼저 락을 획득하는 기능
...
//출금처리 메서드(출금 성공 true, 출금 실패 false 반환)
	public boolean withdraw(int money) {
		boolean chk = false;
		
		//try ~ catch 블럭을 사용할 경우에는 unlock() 메서드 호출은 finally블럭에서 하도록한다.
		try {
			lock.lock();
			if(balance >= money) {
