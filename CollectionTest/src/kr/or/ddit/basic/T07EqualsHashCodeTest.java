...
public class T07EqualsHashCodeTest {
	public static void main(String[] args) {
		/*
		 ...
		 equals()와 hashCode() 메소드를 호출한다.
		 그래서 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의해야한다.
		 ...
		 즉, 객체 a, b가 같아면 a.equals(b)와 b.equals(a)는 둘 다 true를 반환해야 한다.
		 3. 두 객체의 hashCode가 같다고 해서 두 객체가 반드시 같은 객체는 아니다.
		 하지만 두 객체가 같으면 반드시 두 객체의 hashCode가 같아야 한다.
		 4.equals()를 재정의하면 반드시 hashCode()도 재정의해야한다.
		 5. hashCode()는 기본적으로 Heap 메모리에 존재하는 각 개체에 대한 메모리 주소 매핑 정보를 기반으로 하는
		 정수값을 반환한다. 그러므로 hashCode()를 재정의하지 않으면 
		 절대로 두 객체가 같은 hashCode를 반환하지 않는다(다른 객체로 간주됨).
		 
		 - hashCode()에서 사용하는 '해싱알고리즘'에서 서로 다른 객체에 대하여 같은 hashCode값을 
		     만들어낼 수 있다. 그래서 객체가 같지 않아도 hashCode가 같을 수 있다.
		 */
		...
		for(Person p : pSet)System.out.println(p.getId() + " : " + p.getName());
		System.out.println("add(p3) 성공여부 : " + pSet.add(p3));

		for(Person p : pSet)System.out.println(p.getId() + " : " + p.getName());
		System.out.println("remove(p2) 성공 여부 : " + pSet.remove(p2));
		//p1과 p2의 해시코드와 equals가 모두 같을 때 내부적으로 p1과 p2를 동일하게 간주하여
		//p2대신 p1을 지워버림
